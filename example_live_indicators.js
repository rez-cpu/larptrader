...
let seriesClose = [];
console.log(
  `Pulling the last ${HISTORICAL_BARS} bars and then waiting for new data. Press CTRL+C to terminate.`
);
...
function onclose(bar) {
  ...
  //     rsi
  //   )} sma30=${dp(sma)} bbands_up=${dp(bbands.upper)} bbands_low=${dp(
  //     bbands.lower
  //   )} bbands_mid=${dp(bbands.middle)}`
  // );
}
